...
def main():
    ...
    if input_path:
        audio_out = audio2audio(device, model.diffusion_ema, model_chunk_size, inference_batch_size, inference_n_steps, load_to_device(device, input_path, input_sample_rate), noise_level, sample_length_multiplier, callback=create_progress_callback(socketio, 'audio2audio'))
    else:
        audio_out = rand2audio(device, model.diffusion_ema, model_chunk_size, inference_batch_size, inference_n_steps, callback=create_progress_callback(socketio, 'rand2audio'))
...
def create_socket_server(args, callback):
    app = Flask(__name__)
